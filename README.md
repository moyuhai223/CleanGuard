...
- **热力图展示已升级**：在柜位分布图中新增按楼层/类型/区域（每10个柜位一组）的区域热力块，可直观看到占用率与异常柜位分布。
...
- 导入向导新增失败数据预览（前 20 条）、一键复制错误、导出回填模板，支持批量修正后再次导入。
- 编辑页新增用品导入预检弹窗与“复制错误”能力，批量粘贴与模板导入均走统一预检流程。
- 用品类别上限已配置化（`T_SystemConfig`），并提供编辑页“用品上限设置”入口可在线调整。
- 柜位分布图新增区域热力块（每 10 柜为一个区域），可同时观察占用率与异常备注分布。
- 全局 UI 采用原生 WinForms 扁平化工业蓝主题（Segoe UI 字体、按钮交互态、DataGridView 现代样式）。
